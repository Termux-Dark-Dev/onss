...
if (isset($_POST['submit'])) {
    ...
    if ($query->rowCount() == 0) {
        $sql = "insert into tbluser(FullName,MobileNumber,Email,Password,acc_detail)Values(:fname,:mobno,:email,:password,:acc_detail)";
        $query = $dbh->prepare($sql);
        $query->bindParam(':fname', $fname, PDO::PARAM_STR);
        $query->bindParam(':email', $email, PDO::PARAM_STR);
        $query->bindParam(':mobno', $mobno, PDO::PARAM_INT);
        $query->bindParam(':acc_detail', $upiID, PDO::PARAM_STR);
        ...
        <div class='toast bg-success' role='alert' aria-live='assertive' aria-atomic='true' data-delay='5000' style='position: fixed; bottom: 10; right: 20; z-index: 9999;'>
  <div class='toast-header d-flex justify-content-between'>
    <strong class='mr-auto'>Sucessfully Signed Up</strong>
    <button type='button' class='ml-2 mb-1 close' data-dismiss='toast' aria-label='Close'>
      <span aria-hidden='true'>&times;</span>
    </button>
  </div>
  <div class='toast-body text-light '>
            Thanks for sign up. You will be redirected to sign in page in <span id='countdown'>5</span> seconds.
  </div>
</div>
      <script>
        $('.toast').toast('show');
        ...
        var countdown = setInterval(function() {
            ...
            if (countdownNumber <= 0) clearInterval(countdown);
        }, 1000);
  ...
  <div class='toast-header d-flex justify-content-between'>
    <strong class='mr-auto'>Failure In Sign Up</strong>
    <button type='button' class='ml-2 mb-1 close' data-dismiss='toast' aria-label='Close'>
      <span aria-hidden='true'>&times;</span>
    </button>
  </div>
  ...
  Something went wrong.
      ...
      <script>
        ...
        <script src='https://code.jquery.com/jquery-3.4.1.min.js'></script>
        <script src='https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js'></script>
        <div class='toast bg-warning' role='alert' aria-live='assertive' aria-atomic='true' data-delay='5000' style='position: fixed; bottom: 10; right: 20; z-index: 9999;'>
  <div class='toast-header d-flex justify-content-between'>
    <strong class='mr-auto'>Failure In Sign Up</strong>
    <button type='button' class='ml-2 mb-1 close' data-dismiss='toast' aria-label='Close'>
      <span aria-hidden='true'>&times;</span>
                ...
                <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-4">
                    <div class="bg-light rounded p-4 p-sm-5 my-4 mx-3">
                        ...
                        <form method="post">
                            ...
                            <div class="form-floating mb-3">
                                <input type="email" value="" name="upid" required="true" class="form-control">
